...
import { Mic, MessageSquare, AlertCircle, Activity } from 'lucide-react'
...
export default function PatientDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const [p, h, a] = await Promise.all([
        ...
        supabase.from('handoffs').select('*').eq('patient_id', id).order('created_at', { ascending: false }).limit(10),
  ...
  const stabilityTone =
    patient.stability_score >= 80 ? 'bg-emerald-50 text-emerald-700' :
    patient.stability_score >= 60 ? 'bg-amber-50 text-amber-700' :
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-700 mt-3">{patient.diagnosis}</p>
        </div>
        <span className={`px-3 py-1.5 rounded-md text-sm font-medium ${stabilityTone}`}>
          ...
          <div className="space-y-2">
            {alerts.map((a) => (
              <div key={a.id} className="text-sm text-slate-700">
                <span className="font-medium">{a.title}</span> — <span className="text-slate-600">{a.message}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card className="p-5 bg-white/60 backdrop-blur-xl">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-medium text-slate-900 text-sm">Recent handoffs ({handoffs.length})</h2>
        </div>
        {handoffs.length === 0 && (
        ...
        <div className="space-y-3">
          {handoffs.map((h) => (
            <div key={h.id} className="border-b border-slate-200/60 pb-3 last:border-0">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-xs text-slate-500">{new Date(h.created_at).toLocaleString()}</p>
                {h.escalation_risk && (
                  <Badge variant={h.escalation_risk === 'critical' ? 'destructive' : 'secondary'} className="text-xs">
                    {h.escalation_risk}
                  </Badge>
                )}
              </div>
              <p className="text-sm text-slate-700 line-clamp-2">{h.raw_transcript}</p>
              {h.hidden_concerns && h.hidden_concerns.length > 0 && (
                <p className="text-xs text-amber-700 mt-1">
                  ⚠ {h.hidden_concerns.length} hidden concern{h.hidden_concerns.length > 1 ? 's' : ''} detected
                </p>
              )}
            </div>
          ))}
